...
public class Waypoint : MonoBehaviour
{
	...
	void Start() {
		waypointRenderer = gameObject.GetComponentInChildren<MeshRenderer> ();
	}
	...
	void Update()
	{
		...
		// Prevents Waypoints from appearing if they are not visible and the player is not next to them
		if (waypointRenderer.isVisible) {
			float distanceFromWaypoint = Mathf.Abs (gameObject.transform.position.x - Camera.main.transform.position.x) +
			                             Mathf.Abs (gameObject.transform.position.z - Camera.main.transform.position.z);
		
			if (distanceFromWaypoint <= 4.1 || distanceFromWaypoint >= 4.3) {
				_state = State.Hidden;
			} else {
				_state = State.Idle;
			}
	...
	public void Click()
	{
		...
		// This will help in backtracking, so that the player can remember
		// where he or she has been
		color_hilight = Color.red;
	}
